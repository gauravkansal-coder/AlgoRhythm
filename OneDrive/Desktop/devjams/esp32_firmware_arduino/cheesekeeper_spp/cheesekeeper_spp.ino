...
#define DEVICE_NAME "SecureLink_Wallet"
#define LED_PIN 2
...
void scramble(char *data) { for (int i = 0; data[i] != '\0'; i++) data[i] ^= SCRAMBLE_KEY; }
...
void handleCommand(const String &payload) {
  ...
  if (strcmp(command, "login") == 0) {
    ...
    if (stored.length() > 0 && received == stored) {
      isAuthenticated = true; setLed(true);
      String frag = prefs.getString("aesFragment", "");
      sendResponse("2", frag.c_str());
    } else { sendResponse("0", "Login Failed"); }
  } else if (strcmp(command, "setPassword") == 0) {
    char pwd[128]; strlcpy(pwd, doc["payload"] | "", sizeof(pwd));
    scramble(pwd); String hash = sha256(pwd);
    prefs.putString("authPassHash", hash); isAuthenticated = true; setLed(true);
    ...
    prefs.putString("aesFragment", frag); sendResponse("0", "Encrypted fragment stored.");
  } else if (strcmp(command, "logout") == 0) {
    isAuthenticated = false; setLed(false); sendResponse("3", "Logout Successful");
  } else { sendResponse("0", "Unknown command"); }
}
...
void loop() {
  ...
  while (SerialBT.available()) {
    ...
    if (c == '\n' || c == '\r') { if (buffer.length() > 0) { handleCommand(buffer); buffer = ""; } }
    else { buffer += c; if (buffer.length() > MAX_PAYLOAD_LENGTH) buffer = ""; }
  }
  delay(10);
}
